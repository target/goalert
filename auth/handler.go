...
func (h *Handler) EndUserSessionTx(ctx context.Context, tx *sql.Tx, id ...string) error {
	...
	if permission.Admin(ctx) {
		...
		_, err = tx.StmtContext(ctx, h.endSessionUser).ExecContext(ctx, permission.NullUserUUID(ctx), sqlutil.UUIDArray(id))
	}
	return err
}
...
func (h *Handler) EndAllUserSessionsTx(ctx context.Context, tx *sql.Tx) error {
	...
	_, err = stmt.ExecContext(ctx, permission.NullUserUUID(ctx), src.ID)
