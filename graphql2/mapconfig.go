...
func MapConfigValues(cfg config.Config) []ConfigValue {
	return []ConfigValue{
		...
		{ID: "Slack.IncludeDetails", Type: ConfigTypeBoolean, Description: "Include alert details in Slack notifications with expandable view.", Value: fmt.Sprintf("%t", cfg.Slack.IncludeDetails)},
...
func ApplyConfigValues(cfg config.Config, vals []ConfigValueInput) (config.Config, error) {
	...
	for _, v := range vals {
		...
		case "Slack.IncludeDetails":
			val, err := parseBool(v.ID, v.Value)
			if err != nil {
				return cfg, err
			}
			cfg.Slack.IncludeDetails = val
		case "Twilio.Enable":
			val, err := parseBool(v.ID, v.Value)
			if err != nil {
				return cfg, err
