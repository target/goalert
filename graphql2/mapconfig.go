...
func MapConfigValues(cfg config.Config) []ConfigValue {
	return []ConfigValue{
		...
		{ID: "Twilio.RCSSenderSID", Type: ConfigTypeString, Description: "The sender SID for RCS messages. Required if RCS is enabled for the MessagingServiceSID.", Value: cfg.Twilio.RCSSenderSID},
...
func ApplyConfigValues(cfg config.Config, vals []ConfigValueInput) (config.Config, error) {
	...
	for _, v := range vals {
		...
		case "Twilio.RCSSenderSID":
			cfg.Twilio.RCSSenderSID = v.Value
