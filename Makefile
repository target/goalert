...
migrate/schema.sql:
	echo "-- This file is auto-generated by "make migrate/schema.sql"; DO NOT EDIT" > migrate/schema.sql
	echo "-- DATA=$(shell sha256sum migrate/migrations/* | sort | sha256sum)" >> migrate/schema.sql
	echo "-- DISK=$(shell ls migrate/migrations | sort | sha256sum)" >> migrate/schema.sql
	echo "-- PSQL=$(shell psql -d postgres://goalert@localhost -XqAtc "select id from gorp_migrations order by id" | sort | sha256sum)" >> migrate/schema.sql
