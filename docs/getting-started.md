...
Additional options are available for running GoAlert in the form of CLI flags. Their corresponding enviornment variable names are listed as well. 
...
| `--data-encryption-key`      | `GOALERT_DATA_ENCRYPTION_KEY`      | Used to generate an encryption key for sensitive data like signing keys. Can be any length.                                                             |
| `--data-encryption-key-old`  | `GOALERT_DATA_ENCRYPTION_KEY_OLD`  | Fallback key. Used for decrypting existing data only.                                                                                                   |
