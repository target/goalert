...
-- DATA=47bbb836c2a75a9b74947c42432a5f1acdb5f8a1d9fe7b673c0cabced0ad87e6  -
-- DISK=e753dc5db11241a6cf02e525363d714453ef0a56a8b5fce00e67c40168c7b139  -
-- PSQL=e753dc5db11241a6cf02e525363d714453ef0a56a8b5fce00e67c40168c7b139  -
--
...
CREATE OR REPLACE FUNCTION public.fn_job_rotation(id uuid)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION public.fn_job_signal()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION public.fn_track_alert_status()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
...
CREATE OR REPLACE FUNCTION public.fn_util_river_job(queue_name text, kind text, id text, args jsonb)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE TRIGGER trg_track_alert_status_update AFTER UPDATE ON public.alerts FOR EACH ROW WHEN ((new.status IS DISTINCT FROM old.status)) EXECUTE FUNCTION fn_track_alert_status();
...
CREATE TABLE imap_filter_rules (
	...
	to_pattern text,
	updated_at timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT imap_filter_rules_check CHECK (from_pattern IS NOT NULL OR subject_pattern IS NOT NULL OR to_pattern IS NOT NULL),
	CONSTRAINT imap_filter_rules_match_mode_check CHECK (match_mode = ANY (ARRAY['exact'::text, 'contains'::text, 'regex'::text])),
	CONSTRAINT imap_filter_rules_pkey PRIMARY KEY (id),
	CONSTRAINT imap_filter_rules_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE CASCADE
);

CREATE INDEX idx_imap_filter_rules_enabled ON public.imap_filter_rules USING btree (enabled) WHERE (enabled = true);
CREATE INDEX idx_imap_filter_rules_service_id ON public.imap_filter_rules USING btree (service_id);
CREATE UNIQUE INDEX imap_filter_rules_pkey ON public.imap_filter_rules USING btree (id);


CREATE TABLE imap_processed_messages (
	message_id text NOT NULL,
	processed_at timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT imap_processed_messages_pkey PRIMARY KEY (message_id)
);

CREATE INDEX idx_imap_processed_messages_processed_at ON public.imap_processed_messages USING btree (processed_at);
CREATE UNIQUE INDEX imap_processed_messages_pkey ON public.imap_processed_messages USING btree (message_id);
...
CREATE TRIGGER trg_pending_signals_after_insert AFTER INSERT ON public.pending_signals FOR EACH ROW EXECUTE FUNCTION fn_job_signal();
...
CREATE TABLE service_imap_config (
	created_at timestamp with time zone DEFAULT now() NOT NULL,
	delete_after boolean DEFAULT false NOT NULL,
	enabled boolean DEFAULT false NOT NULL,
	host text DEFAULT 'imap.gmail.com'::text NOT NULL,
	include_body boolean DEFAULT true NOT NULL,
	include_from boolean DEFAULT true NOT NULL,
	include_headers boolean DEFAULT false NOT NULL,
	include_subject boolean DEFAULT true NOT NULL,
	include_to boolean DEFAULT true NOT NULL,
	...
	username text NOT NULL,
	CONSTRAINT service_imap_config_pkey PRIMARY KEY (service_id),
	CONSTRAINT service_imap_config_poll_interval_minutes_check CHECK (poll_interval_minutes > 0 AND poll_interval_minutes <= 1440),
	CONSTRAINT service_imap_config_service_id_fkey FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE CASCADE
);

CREATE INDEX idx_service_imap_config_enabled ON public.service_imap_config USING btree (enabled) WHERE (enabled = true);
CREATE UNIQUE INDEX service_imap_config_pkey ON public.service_imap_config USING btree (service_id);
