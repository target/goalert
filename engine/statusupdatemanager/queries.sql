-- name: StatusMgrForcedStatusUpdates :exec
UPDATE
...
-- name: StatusMgrCleanupSubscriptions :exec
...
-- name: StatusMgrNextSubscription :one
SELECT
    sub.id,
    channel_id,
    contact_method_id,
...
-- name: StatusMgrInsertUserCMMessage :exec
INSERT INTO outgoing_messages (id, message_type, contact_method_id, user_id, alert_id, alert_log_id)
    VALUES (@msg_id::uuid, 'alert_status_update', @cm_id::uuid, @user_id::uuid, @alert_id::bigint, @alert_log_id);

-- name: StatusMgrInsertChanMessage :exec
INSERT INTO outgoing_messages (id, message_type, channel_id, alert_id, alert_log_id)
    VALUES (@msg_id::uuid, 'alert_status_update', @channel_id::uuid, @alert_id::bigint, @alert_log_id);

-- name: StatusMgrLastLog :one
SELECT
    id,
    sub_user_id
FROM
