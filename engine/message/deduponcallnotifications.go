...
// dedupOnCallNotifications will remove old on-call notifications if a newer one exists for the same alert/destination.
func dedupOnCallNotifications(messages []Message) ([]Message, []string) {
	...
	for _, msg := range messages {
		if msg.Type != notification.MessageTypeScheduleOnCallStatus {
