...
func NewDB(ctx context.Context, db *sql.DB) (*DB, error) {
	...
	return &DB{
		...
		schedData:    p.P(`select schedule_id, data from schedule_data where data notnull and (last_cleanup_at isnull or last_cleanup_at < now() - '1 day'::interval) for update limit 100`),
