...
func (p *Engine) sendMessage(ctx context.Context, msg *message.Message) (*notification.SendResult, error) {
	...
	case notification.MessageTypeAlertStatus:
		...
		if stat == nil {
			return nil, fmt.Errorf("could not find original notification for alert %d to %v", msg.AlertID, msg.Dest)
		}
	...
	if isFirstAlertMessage && res.State.IsOK() {
		...
		if err != nil {
			...
			log.Log(ctx, fmt.Errorf("track status updates for alert #%d for %v: %w", msg.AlertID, msg.Dest, err))
		}
	}
