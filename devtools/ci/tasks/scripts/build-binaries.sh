...
make check test $BINARIES BUNDLE=1 BUILD_FLAGS=-trimpath
...
make ../darwin/goalert/bin/goalert BUNDLE=1 GOOS=darwin BIN_DIR=../darwin/goalert/bin BUILD_FLAGS=-trimpath
...
then
    ...
    tar czvf ../bin/integration-${BVERSION}-linux-amd64.tgz -C .. goalert/bin goalert/cypress goalert/cypress.json goalert/COMMIT goalert/devtools/ci goalert/.git/resource
fi
