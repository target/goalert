...
readarray -t EXCLUDE_GENERATED_FILES_ARGS < <(git grep -l -e 'generated by .* DO NOT EDIT' | filter_nongen_files | sed 's/^/:\(exclude\)/')
...
readarray -t EXCLUDE_IGNORED_FILES_ARGS < <(cat "$IGNORE_PATTERN_FILE" 2>/dev/null | resolve_git_files | sed 's/^/:\(exclude\)/')
...
OUTPUT=$(git diff --ignore-blank-lines --numstat -w --diff-filter=d --minimal --merge-base "origin/$MAIN_BRANCH" -- "${EXCLUDE_GENERATED_FILES_ARGS[@]}" "${EXCLUDE_IGNORED_FILES_ARGS[@]}")
